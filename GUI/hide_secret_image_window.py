...
def hide_message(secret_message_entry,generated_image_label, mode):
    ...
    new_image = PhotoImage(file=loaded_image_path)
    generated_image_label.configure(image=new_image)
    generated_image_label.image = new_image
...
def open_hide_secret_message_window(root_window):
    ...
    generated_image = Label(right_frame, image=image)
    ...
    Button(tool_bar, text="After EOF", command=lambda:hide_message(secret_message_entry,generated_image,"EOF")).grid(row=4, column=0, padx=5, pady=5, sticky='ew', columnspan=2)
